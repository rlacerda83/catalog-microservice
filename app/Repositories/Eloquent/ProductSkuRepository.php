...
class ProductSkuRepository extends AbstractRepository
{
    ...
    public function __construct(App $app)
    {
        ...
        $this->tableSkuAtributes = Sku\Attribute::getTableName();
    ...
    {
        $key = md5('attributes_sku' . $idSku);
        $query = $this->getModel()->newQuery();

        $fields = ["{$this->tableVariationAttribute}.*"];

        $query->select($fields)
            ->join($this->tableSkuAtributes, "{$this->tableSkuAtributes}.sku_id", '=', "{$this->tableSku}.id")
            ->join($this->tableVariationAttribute, "{$this->tableVariationAttribute}.id", '=', "{$this->tableSkuAtributes}.attribute_id")
